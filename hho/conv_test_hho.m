function  hho()
    ...
    for deg = 0:4
        ...
        convrate = log(err(deg+1,1)/err(deg+1,2))/log(h(deg+1,1)/h(deg+1,2));
        disp(sprintf('Convergence rate for K = %d: %g', deg, convrate));
    end
end
...
% Compute error with mass matrix
...
% Compute error with integral
function error = compute_error_int(pd, ii, fun, dofs)
    [nodes,weights] = gauss_quadrature(2*pd.K, pd.h, ii);
    ...
    for wi = 1:length(weights)
        ...
        error = error + weights(wi) * (dot(phi,dofs) - fun(nodes(wi)))^2;
    end
end
...
function globerr = hho(pd, loadfun, solfun)
    ...
    for ii = 1:pd.N
        ...
        rdofs = GT*[solT;solF];
        rdofs = [solT(1);rdofs];
        error = error + compute_error_int(pd, ii, solfun, rdofs);
    end
